...
const char* kTitle =
    "Case 06501550 repro | 1 = Left: perlinA (float 23.53 & float 44.0) | "
    "2 = Right: perlinB (uniform 23.53 & uniform 44.0)          Move with arrow keys";
...
// The original OpenGL shader had four uniforms:
//   float value_U  = 23.53;
//   float value_U2 = 44.0;
//   vec2  gridOffset;
//   float zoomLevel;
// Vulkan push constants are used to preserve the same per-draw state.
...
{
    return fract(sinM(dot(inCoord, vec2(pc.value_U, pc.value_U2))) * 42350.45);
}
...
void main()
{
    ...
    vec2 inCoord = gridUV * (16.0 * pc.zoomLevel) - pc.gridOffset;
...
struct VulkanContext {
    ...
    // State corresponding to the original OpenGL gridOffset uniform.
...
void recordCommandBuffer(VulkanContext& ctx, uint32_t imageIndex)
{
    ...
    pc.gridOffsetX = ctx.gridOffsetX;
    pc.gridOffsetY = ctx.gridOffsetY;
    pc.valueU = 23.53f;
    pc.valueU2 = 44.0f;
    pc.zoomLevel = ctx.zoomLevel;

    vkCmdPushConstants(cmd, ctx.pipelineLayout, VK_SHADER_STAGE_FRAGMENT_BIT,
                       0, sizeof(pc), &pc);
...
int main()
{
    try {
        ...
        while (!glfwWindowShouldClose(window)) {
            ...
            const auto timeDelta = now - prefFrameTime;

            // Preserve the original timing calculation exactly:
            // speed = 16.0 * (duration.count() / speed1)
            const float speed =
                16.0f * (static_cast<double>(timeDelta.count()) / speed1);
            ...
            if (glfwGetKey(window, GLFW_KEY_EQUAL) == GLFW_PRESS)
                zoomLevel = std::min(zoomLevel + 0.01f, 10.0f);

            if (glfwGetKey(window, GLFW_KEY_MINUS) == GLFW_PRESS)
                zoomLevel = std::max(zoomLevel - 0.01f, 1.0f);

            ctx.gridOffsetX = offsetX;
            ctx.gridOffsetY = offsetY;
